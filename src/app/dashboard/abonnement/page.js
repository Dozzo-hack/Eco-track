...
export default function AbonnementPage() {
  ...
  const [dbPrices, setDbPrices] = useState({
    eco: { price: 25, desc: "Pour les petits foyers" },
  ...
  const handleFinalPayment = async () => {
    ...
    if (!phoneNumber || phoneNumber.trim().length !== 9) {
      Swal.fire({
        ...
        text: "Veuillez entrer un numéro de téléphone camerounais valide à 9 chiffres.",
        icon: "warning",
    ...
    try {
      ...
      if (data.success) {
        ...
        // Notification stylisée invitant le client à composer son code PIN sur son mobile
        Swal.fire({
          title: "Saisie de code PIN requise",
          text: "Un prompt vient de s'afficher sur votre téléphone. Veuillez saisir votre code secret Mobile Money pour confirmer le paiement.",
          icon: "info",
          confirmButtonColor: "#6200ee",
          customClass: { popup: "rounded-[35px]" }
        });
        ...
        Swal.fire({
          ...
          text: data.message || "Vérifiez vos configurations API Campay.",
          icon: "error",
          confirmButtonColor: "#ef4444",
          customClass: { popup: "rounded-[35px]" }
        });
  ...
  if (loadingPrices) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center space-y-4">
        ...
        <p className="text-zinc-500 font-bold tracking-tight animate-pulse">Synchronisation de l'écosystème financier...</p>
      </div>
    );
  }
    ...
    <div className="pt-24 lg:pt-0 pb-24 space-y-10 min-h-screen animate-in fade-in duration-500 max-w-7xl mx-auto px-4">
      ...
      <div className="bg-emerald-50 border-l-4 border-emerald-500 p-5 rounded-3xl shadow-sm">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-xs font-black text-emerald-900 uppercase tracking-wider">Paiement Mobile Sécurisé via Campay</p>
            <p className="text-xs font-bold text-emerald-700 leading-relaxed mt-0.5">
              Tous nos abonnements sont traités localement et de manière sécurisée au Cameroun. Vous pouvez payer par **Orange Money** ou **MTN MoMo** directement depuis votre téléphone.
            </p>
          </div>
        </div>
      </div>

      {/* --- GUICHET DE PAIEMENT CAMPAY MODIFIÉ --- */}
      {showPayModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-[35px] p-8 max-w-md w-full shadow-2xl border border-gray-100 relative overflow-hidden">
            ...
            <p className="text-xs text-gray-500 font-bold mb-6 uppercase tracking-wider">Formule {selectedPlan?.name} • {selectedPlan?.amount.toLocaleString()} XAF net</p>

            <div className="space-y-5">
              
              <div>
                <label className="block text-xs font-black uppercase text-zinc-400 mb-2.5">1. Choisissez votre opérateur</label>
                <div className="grid grid-cols-2 gap-3">
                  <button 
                    type="button"
                    onClick={() => setOperateur("Orange Money")}
                    className={`p-4 rounded-xl border-2 font-black text-xs flex items-center justify-center gap-1.5 transition-all active:scale-95 ${operateur === "Orange Money" ? "border-orange-500 bg-orange-50 text-orange-600" : "border-gray-200 text-gray-700 hover:bg-zinc-50"}`}
                  >
                    🍊 Orange Money
                  </button>
                  <button 
                    ...
                    className={`p-4 rounded-xl border-2 font-black text-xs flex items-center justify-center gap-1.5 transition-all active:scale-95 ${operateur === "MTN MoMo" ? "border-yellow-500 bg-yellow-50 text-yellow-700" : "border-gray-200 text-gray-700 hover:bg-zinc-50"}`}
                  >
                    💛 MTN MoMo
                  </button>
                </div>
              ...
              <div className="flex gap-4 pt-4">
                ...
                >
                  {isProcessing ? "Envoi du Prompt..." : `Confirmer`}
                </button>
              </div>
            </div>
          </div>
