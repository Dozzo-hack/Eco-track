...
import { 
  DollarSign, Download, Clock, CreditCard, TrendingUp, BarChart3, X, ChevronRight, PieChart 
} from 'lucide-react';
...
export default function AdminFinancePage() {
  ...
  const [financeData, setFinanceData] = useState({
    ...
    analytics: { 
      totalTrimestre: 0, 
      panierMoyen: 0,
      previsions: { mois: "Avril", valeurRetenue: 0, tauxCroissanceEstime: "0%" } 
    },
    fluxRecents: [],
    pricing: []
  });
  ...
  const handleExportData = () => {
    ...
    link.setAttribute("download", `EcoTrack_Finances_CamPay_${new Date().toISOString().slice(0,10)}.csv`);
  ...
  return (
    <div className="relative min-h-screen bg-black text-white p-6">
      ...
      <div className="flex justify-between items-center mb-10">
        <div>
          <h2 className="text-4xl font-black text-white italic tracking-tighter">FINANCES (CAMPAY)</h2>
          <p className="text-zinc-500 text-[10px] font-black uppercase tracking-[0.3em]">Flux monétaires mobiles via CamPay</p>
        </div>
        <button onClick={handleExportData} className="flex items-center gap-2 bg-zinc-900 border border-zinc-800 text-white px-6 py-3 rounded-2xl font-black text-[10px] uppercase hover:bg-white hover:text-black transition-all">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
        <div onClick={() => setShowStats(true)} className="bg-zinc-900 border border-zinc-800 p-8 rounded-[40px] relative cursor-pointer hover:border-green-500/50 transition-all">
          <p className="text-zinc-500 text-[9px] font-black uppercase tracking-widest mb-2 flex items-center gap-2"><BarChart3 size={12}/> Chiffre d'Affaire</p>
          ...
          <div className="mt-4 text-green-500 text-[10px] font-black uppercase italic flex items-center gap-2"><TrendingUp size={14} /> Prévisions et Droite de Tendance</div>
        </div>
        ...
        <div className="bg-zinc-900 border border-zinc-800 p-8 rounded-[40px]">
          ...
          <div className="mt-4 text-orange-500 text-[10px] font-black uppercase italic flex items-center gap-2"><Clock size={14} /> Écoute Webhook active</div>
        </div>

        <div className="bg-green-500 p-8 rounded-[40px] flex flex-col justify-between">
          <div>
            <p className="text-black/60 text-[9px] font-black uppercase tracking-widest mb-1 italic">Solde Net Estimé</p>
          ...
          <button className="mt-6 bg-black text-white w-full py-4 rounded-2xl text-[10px] font-black uppercase">Demander un reversement</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-zinc-950 border border-zinc-900 rounded-[45px] p-10">
          <h4 className="text-xl font-black text-white italic uppercase tracking-tighter mb-8 flex items-center gap-3"><CreditCard className="text-green-500" size={24} /> Flux Récents</h4>
            ...
            {financeData.fluxRecents.map((tx) => (
              <div key={tx.reference} className="flex items-center justify-between p-6 bg-zinc-900/30 rounded-3xl border border-zinc-800/50">
                <div className="flex items-center gap-5">
                  ...
                  <div>
                    <p className="font-black text-sm text-white uppercase">{tx.userName}</p>
                    <p className="text-[10px] text-zinc-600 font-bold">{new Date(tx.createdAt).toLocaleDateString()} • {tx.operateur}</p>
                  </div>
                </div>
        ...
        <div className="bg-zinc-900/50 border border-zinc-800 rounded-[45px] p-10">
          <h4 className="text-xl font-black text-white italic uppercase tracking-tighter mb-8">Grille Grille</h4>
          <div className="space-y-4">
            {financeData.pricing?.map((plan) => (
              <div key={plan.planId} className="p-6 bg-black border border-zinc-800 rounded-[2.5rem] flex justify-between items-center">
                ...
                <button onClick={() => { setEditingPlan(plan.planId); setNewPriceValue(plan.price); }} className="p-3 bg-zinc-900 rounded-xl text-green-500 hover:bg-green-500 hover:text-black transition-all">
      ...
      {editingPlan && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md">
          <div className="bg-zinc-900 border border-zinc-800 p-8 rounded-3xl max-w-sm w-full">
            ...
            <input type="number" value={newPriceValue} onChange={(e) => setNewPriceValue(e.target.value)} className="w-full bg-black border border-zinc-700 rounded-xl py-3 px-4 text-white font-bold mb-4 outline-none focus:border-green-500" />
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-zinc-950 border border-zinc-800 w-full max-w-4xl rounded-[50px] p-10 shadow-2xl">
            ...
            <div className="mb-12">
              ...
              <p className="text-xs text-zinc-500 font-bold uppercase tracking-widest mt-1">Modèles prévisionnels basés sur l'historique CamPay</p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-12 gap-10 items-end">
                ...
                {financeData.graphique.map((data, idx) => (
                  <div key={idx} className="flex flex-col items-center gap-5 w-1/4 group">
                    <div className="relative w-full flex justify-center">
                      <div className="w-12 bg-zinc-800 rounded-2xl group-hover:bg-green-500 h-48 relative flex items-end overflow-hidden">
              ...
              <div className="md:col-span-4 space-y-6">
                <div className="p-6 bg-zinc-900/50 rounded-3xl border border-zinc-800">
                  ...
                  <p className="text-3xl font-black text-green-500 italic">{financeData.analytics.previsions?.tauxCroissanceEstime || "0%"}</p>
                </div>
                <div className="p-6 bg-zinc-900/50 rounded-3xl border border-zinc-800">
            ...
            <div className="mt-10 pt-10 border-t border-zinc-900 flex justify-between items-center">
              <div className="flex gap-10">
                ...
                <div>
                  <p className="text-[8px] font-black text-zinc-600 uppercase mb-1">Projection ({financeData.analytics.previsions?.mois || "Suivant"})</p>
                  <p className="text-xl font-black text-green-500 italic tracking-tighter">{(financeData.analytics.previsions?.valeurRetenue || 0).toLocaleString()} FCFA</p>
                </div>
              </div>
